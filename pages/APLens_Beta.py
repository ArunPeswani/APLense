...
st.set_page_config(page_title="APLens Beta - Advanced Suite", page_icon="🧪", layout="wide")
...
app_mode = st.sidebar.radio("Navigation", nav_options, key=f"nav_beta_{rc}")

st.sidebar.markdown("---")

st.sidebar.subheader("Analysis Settings")
min_words = st.sidebar.slider("Minimum N-Gram Words", 1, 10, 4, key=f"min_beta_{rc}")
max_words = st.sidebar.slider("Maximum N-Gram Words", 1, 10, 6, key=f"max_beta_{rc}")
similarity_threshold = st.sidebar.slider("🚨 Flagging Threshold (%)", 10, 100, 40, 5, key=f"thresh_beta_{rc}")
...
if st.session_state.logged_in:
    ...
    st.sidebar.info("💡 **Sign in** via the top-right button to enable automated report history storage.")
...
if app_mode == "Plagiarism Checker":
    ...
    course_assignment_name = st.text_input("📚 Course Name / Assignment Title", placeholder="e.g., CS101 - Research Paper", key=f"course_beta_{rc}")
    
    upload_choice = st.radio("Select Upload Type", ["Individual Files", "Direct Folder Selection", "ZIP Archive (.zip)"], key=f"up_beta_{rc}")
    
    processed_files = []
    supported_exts = ("docx", "pdf", "txt", "rtf", "md", "xlsx", "xls")

    if upload_choice == "Individual Files":
        raw_files = st.file_uploader("Upload Student Submissions", type=list(supported_exts), accept_multiple_files=True, key=f"indiv_beta_{rc}")
        if raw_files: processed_files = raw_files
    elif upload_choice == "Direct Folder Selection":
        dir_files = st.file_uploader("Select Folder", type=list(supported_exts), accept_multiple_files="directory", key=f"dir_beta_{rc}")
        if dir_files: processed_files = [f for f in dir_files if f.name.lower().endswith(supported_exts) and '__MACOSX' not in f.name]
    else:
        zip_file = st.file_uploader("Upload ZIP Archive", type=["zip"], key=f"zip_beta_{rc}")
        if zip_file:
            try:
                    ...
                    for filename in z.namelist():
                        if filename.lower().endswith(supported_exts) and not filename.startswith('__MACOSX/'):
                            with z.open(filename) as f:
                                ...
                                if file_bytes.name: processed_files.append(file_bytes)
            except Exception as e:
                st.error(f"Error reading ZIP: {e}")

    if processed_files:
        st.info(f"Loaded {len(processed_files)} file(s).")
        col1, col2 = st.columns(2)
        with col1: run_std = st.button("Run Plagiarism Analysis", type="primary", key=f"run_std_beta_{rc}")
        with col2: run_para = st.button("🔍 Run Paraphrase Analysis", type="secondary", key=f"run_para_beta_{rc}")

        if run_std or run_para:
            if len(processed_files) < 2:
                st.error("Please upload at least 2 files.")
            else:
                mode_label = "Paraphrased Plagiarism Analysis" if run_para else "Standard Plagiarism Analysis"
                documents, filenames = [], []
                ...
                if len(documents) < 2:
                    st.error("Insufficient text in documents.")
                else:
                    n = len(documents)
                    if run_para:
                        ...
                        for i in range(n):
                            for j in range(n):
                                if i == j: sim_matrix[i][j] = 100.0
                                else:
                                    s = difflib.SequenceMatcher(None, documents[i].lower(), documents[j].lower()).ratio()
                                    words1, words2 = set(documents[i].lower().split()), set(documents[j].lower().split())
                                    jaccard = len(words1.intersection(words2)) / max(len(words1.union(words2)), 1)
                                    sim_matrix[i][j] = min(round(((s * 0.5) + (jaccard * 0.5)) * 140, 2), 100.0)
                    else:
                        vectorizer = TfidfVectorizer(stop_words='english', ngram_range=(min_words, max_words), max_features=10000)
                        sim_matrix = (cosine_similarity(vectorizer.fit_transform(documents)) * 100).tolist()

                    df = pd.DataFrame(sim_matrix, index=filenames, columns=filenames)
                    st.session_state.beta_df = df
                    st.session_state.beta_matrix = sim_matrix
                    st.session_state.beta_filenames = filenames
                    st.session_state.beta_analyzed = True
                    st.session_state.beta_run_label = mode_label
    ...
    if st.session_state.get("beta_analyzed", False):
        ...
        sim_matrix = st.session_state.beta_matrix
        filenames = st.session_state.beta_filenames
        run_label = st.session_state.get("beta_run_label", "Analysis")
        course_name = st.session_state.get("beta_course", "Report")

        st.success(f"{run_label} Complete for **{course_name}**!")
        
        flat_scores = [sim_matrix[i][j] for i in range(len(filenames)) for j in range(len(filenames)) if i != j]
        flagged_count = sum(1 for score in flat_scores if score >= similarity_threshold)

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("📁 Files Scanned", len(filenames))
        m2.metric("📈 Max Similarity", f"{max(flat_scores) if flat_scores else 0:.1f}%")
        m3.metric("📊 Avg Similarity", f"{sum(flat_scores)/len(flat_scores) if flat_scores else 0:.1f}%")
        m4.metric(f"🚨 Flagged (≥{similarity_threshold}%)", flagged_count)

        fig = go.Figure(data=go.Heatmap(
            z=sim_matrix, x=filenames, y=filenames,
            colorscale="Reds" if "Paraphrase" not in run_label else "Oranges", zmin=0, zmax=100
        ))
        fig.update_layout(height=650, margin=dict(l=150, r=50, t=50, b=150), xaxis=dict(tickangle=-45), yaxis=dict(autorange='reversed'))
        st.plotly_chart(fig, use_container_width=True)

        st.dataframe(df.style.format("{:.2f}%"))
        
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer: df.to_excel(writer, sheet_name='Report')
        st.download_button("📥 Download Excel Report", output.getvalue(), file_name=f"plagiarism_report_{course_name.replace(' ', '_')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
# ==========================================
    ...
    st.write("Compare two files or spreadsheets side-by-side.")
    
    c1, c2 = st.columns(2)
    with c1: f1 = st.file_uploader("Student A Document", type=["docx", "pdf", "txt", "xlsx", "xls"], key=f"d1_beta_{rc}")
    with c2: f2 = st.file_uploader("Student B Document", type=["docx", "pdf", "txt", "xlsx", "xls"], key=f"d2_beta_{rc}")
    
    if f1 and f2:
        if st.button("Run Deep Dive Matcher", type="primary"):
            st.info("Deep Dive comparison executed successfully in Beta Sandbox.")
...
# ==========================================
    ...
    if not st.session_state.logged_in:
        st.warning("🔒 Please sign in using the top-right **Sign In** button to view your saved history.")
    else:
        if not st.session_state.saved_reports:
            st.info("No reports saved yet. Run an analysis with report saving enabled!")
        else:
            if st.button("🗑️ Clear History"):
                st.session_state.saved_reports = []
                st.rerun()
            for idx, rep in enumerate(reversed(st.session_state.saved_reports)):
                with st.expander(f"📌 [{rep['timestamp']}] {rep['course']} — {rep['type']} (Expires: {rep['expiry']})"):
