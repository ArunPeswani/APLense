...
st.set_page_config(page_title="APLens Beta - Google Profile Header", page_icon="🧪", layout="centered")
...
with header_col2:
    if st.user.is_logged_in:
        # Render a clean, Google-style profile header layout in the top right
        profile_col1, profile_col2 = st.columns([0.7, 0.3])
        with profile_col1:
            st.markdown(f"""
                <div style="display: flex; align-items: center; justify-content: flex-end; gap: 10px; padding-top: 10px;">
                    ...
                    <div style="text-align: left; line-height: 1.2;">
                        <div style="font-weight: 600; font-size: 13px; color: #202124;">{user_name}</div>
                        <div style="font-size: 11px; color: #5f6368;">{user_email}</div>
                    </div>
                </div>
...
if st.user.is_logged_in:
    ...
    st.write(f"Welcome, {user_name}! Your Google profile image and native session are fully active.")
    
    st.info("You have full access to the Beta suite features.")
else:
    st.warning("🔒 Please sign in using the top-right login button to unlock the Beta suite.")
