...
st.set_page_config(page_title="APLens Beta - Google Auth", page_icon="🧪", layout="centered")
...
with header_col2:
    if st.user.is_logged_in:
        ...
        # Logged Out State: Clean Google Pill Button inside a Popover
        with st.popover("Sign in with Google"):
            st.markdown("""
                <div style="display: flex; align-items: center; justify-content: center; gap: 10px; padding: 4px 0px;">
                    ...
                    <span style="font-family: Roboto, sans-serif; font-weight: 500; font-size: 14px; color: #3c4043;">Sign in with Google</span>
                </div>
            """, unsafe_allow_html=True)
            
            st.markdown("<div style='margin-top: 8px;'></div>", unsafe_allow_html=True)
            if st.button("Click to Authenticate", type="primary", use_container_width=True):
                st.login("google")
...
if st.user.is_logged_in:
    ...
    st.warning("🔒 Please sign in using the top-right **Sign in with Google** button to unlock the Beta suite.")
