...
if "code" in query_params:
    ...
    try:
        # Exchange the auth code directly for a Supabase session
        session_data = supabase.auth.exchange_code_for_session({"auth_code": auth_code})
        if session_data and session_data.user:
            st.session_state.logged_in = True
            st.session_state.user_email = session_data.user.email or ""
            st.query_params.clear()
            st.rerun()
    except Exception as e:
        st.error(f"Sign-in failed during exchange: {e}")
        st.query_params.clear()
    ...
    if st.session_state.logged_in:
        ...
        with st.popover("🔐 Account Login"):
            ...
            with auth_tab_social:
                ...
                try:
                    # Generate OAuth URL with flow_type PKCE
                    ...
                    github_auth = supabase.auth.sign_in_with_oauth({
                        "provider": "github",
                        "options": {
                            "redirect_to": redirect_target,
                            "flow_type": "pkce"
                    ...
                    github_url = github_auth.url

                    st.markdown(f"""
                        <div class="login-container">
                            <a href="{google_url}" target="_top" class="social-login-btn">
                            ...
                            <a href="{github_url}" target="_top" class="social-login-btn">
