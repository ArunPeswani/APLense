...
st.set_page_config(page_title="APLens Beta - Account Menu", page_icon="🧪", layout="centered")
...
with header_col2:
    if st.user.is_logged_in:
        # Clean layout: Profile Avatar + Popover Menu for Sign Out
        menu_col1, menu_col2 = st.columns([0.3, 0.7])
        with menu_col1:
            st.markdown(f"""
            ...
            with st.popover("Account"):
                st.markdown(f"""
                    <div style="text-align: center; padding: 10px 0px;">
                        <img src="{user_avatar}" style="width: 56px; height: 56px; border-radius: 50%; border: 2px solid #1a73e8; object-fit: cover; margin-bottom: 6px;">
